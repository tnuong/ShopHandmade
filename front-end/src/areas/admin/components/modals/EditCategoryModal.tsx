...
const EditCategoryModal: FC<CreateCategoryModalProps> = ({
    handleOk,
    categories,
    category
}) => {
    ...
    useEffect(() => {
        if (category) {
            form.setFieldsValue({
                parentCategoryId: category?.parentCategory?.id ?? 0,
                name: category?.name,
    ...
    const onFinish: FormProps<CategoryRequest>['onFinish'] = async (values) : Promise<void> => {
